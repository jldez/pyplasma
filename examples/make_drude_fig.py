...
if __name__ == '__main__':
	...
	ax3.set_xlabel(r"$t$",horizontalalignment='right', x=1.12,labelpad=-20)
	...
	# Scaling of gamma_ib vs gamma
	def f1(gamma):
		return 2.0*gamma*f2(gamma)

	ax4.semilogx(gammas,f2(gammas),color="darkred",lw=2,label=r"$\mathcal{E}_p$")
	ax4.semilogx(gammas,f1(gammas),color=colors[2],lw=2,label=r"$\gamma_\mathrm{ib}$")
	...
	ax4.set_xlabel(r"$\gamma/\omega$",horizontalalignment='right', x=1.055,labelpad=-12)
	...
	plt.tight_layout()
	plt.subplots_adjust(hspace = 0.17,wspace = 0.2,left=0.06,right=0.91,bottom=0.06,top=0.99)
