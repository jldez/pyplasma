...
if __name__ == "__main__":
    ...
    for n in range(N):
        ...
        last_results = f'growth_{n-1}.npy' if n>0 else None

        command = f'python3 {FILENAME}/ablation_3D.py '
        command += f'-o=growth_{n}.npy '
        command += f'-F={F} '
        command += f'-t={tau} '
    ...
    for n in range(N):
        ...
        results = np.load(f'growth_{n}.npy', allow_pickle=True).item()
    ...
    plt.tight_layout()
    # plt.show()
    filename = f'growth.pdf'
    plt.savefig(filename)
    os.system(f'pdfcrop {filename} {filename} > /dev/null')