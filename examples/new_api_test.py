...
def example_3D():
  ...
  dom.add_observer(Observer('E', 'watch', x=440*nm, vlim=(-laser.E0*1.1, laser.E0*1.1), keep_pml=True, out_step=1))
  dom.add_observer(Observer('Ez', 'watch', y=1*um, z=1*um, vlim=(-laser.E0*1.1, laser.E0*1.1), keep_pml=True, out_step=1))
  dom.add_observer(Observer('rho', 'watch', x=440*nm, vlim=(0,material.density), out_step=1))
...
def example_1D():
  ...
  laser = Laser(wavelength=800*nm, pulse_duration=10*fs, fluence=6e4, t0=25*fs, phase=True)
  dom.add_laser(laser, position='default', source_mode='tfsf')

  material = Material(
                    index=1.6, resonance=120e-9, chi3=2e-22,
                    drude_params={'damping':1e15, 'm_VB':4},
                    ionization_params={'rate_equation':'sre','bandgap':9.*c.e,'density':2.2e28,'alpha_sre':0.001}
                    # ionization_params={'rate_equation':'mre','bandgap':9.*c.e,'density':2e28,'cross_section':1e-19}
                    # ionization_params={'rate_equation':'dre','bandgap':9.*c.e,'density':2e28,'cross_section':1e-19}
  ...
  dom.add_observer(Observer('E', 'watch', keep_pml=True, out_step=5))
  # dom.add_observer(Observer('Jf', 'watch', keep_pml=True, out_step=5))
